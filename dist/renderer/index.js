...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const leftButton = document.querySelector('.block .left');
    const rightButton = document.querySelector('.block .right');
...
function monthloader(thisYear, thisMonth) {
    ...
    const day = document.querySelectorAll('.block .content .days button');
    day.forEach((button) => {
    ...
    const days = document.querySelector('.block .content .days');
    // исключение для первого дня месяца, если он приходится на воскресенье (0), то нужно отобразить 6 пустых кнопок перед первым днем
    if (firstDayOfMonth == 0) {
        ...
        // остальные случаи, когда первый день месяца приходится на понедельник (1) - субботу (6), нужно отобразить соответствующее количество пустых кнопок перед первым днем
    }
    else {
